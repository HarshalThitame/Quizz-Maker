...
public class RecruiterLoginServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            String pass = request.getParameter("rcpassword");

            out.println(email);
            out.println(pass);
            RecruiterDao dao = new RecruiterDao(ConnectionProvider.getConnection());

            Recruiters r = dao.getRecruiterByEmailAndPassword(email, pass);

            if (r != null) {
                HttpSession s = request.getSession();
                s.setAttribute("currentRecruiter", r);
